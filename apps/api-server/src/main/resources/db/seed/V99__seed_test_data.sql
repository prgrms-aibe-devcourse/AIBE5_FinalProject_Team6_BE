...
--    H2: DATE_ADD(NOW(), INTERVAL -N DAY) / DATE_ADD(NOW(), INTERVAL -N HOUR)

-- NOVA 피드 5개
INSERT INTO artist_feed (artist_id, artist_member_id, content, like_count, comment_count, created_at)
VALUES
    (1, 1, 'NOVA 팬 여러분, 안녕하세요! 드디어 컴백 준비가 시작됐어요.',  12,  3, DATE_ADD(NOW(), INTERVAL -5 DAY)),
    (1, 1, '오늘 뮤직비디오 촬영 완료! 기대해주세요.',                    27,  8, DATE_ADD(NOW(), INTERVAL -3 DAY)),
    (1, 1, '새 앨범 타이틀곡 작업 중. 힌트: 여름 느낌 물씬~',              9,  1, DATE_ADD(NOW(), INTERVAL -1 DAY)),
    (1, 1, '오늘 라이브 방송 22:00 KST 시작합니다! 기다려줘서 고마워요.', 45, 15, DATE_ADD(NOW(), INTERVAL -12 HOUR)),
    (1, 1, '드롭스 굿즈 최종 디자인 확정됐어요. 곧 공개 예정.',             6,  0, DATE_ADD(NOW(), INTERVAL -1 HOUR)),
    (1, 2, '세라입니다! 오늘 연습 끝나고 팬분들 생각하며 글 남겨요 💜',    8,  2, DATE_ADD(NOW(), INTERVAL -8 HOUR));

-- LUNA 피드 3개 (artist_member_id=5 은별)
INSERT INTO artist_feed (artist_id, artist_member_id, content, like_count, comment_count, created_at)
VALUES
    (2, 5, 'LUNA 1주년을 함께해줘서 정말 감사해요. 팬 여러분 최고!', 33, 11, DATE_ADD(NOW(), INTERVAL -4 DAY)),
    (2, 5, '포토카드 세트 패키지 디자인 비하인드 공개.',               18,  5, DATE_ADD(NOW(), INTERVAL -2 DAY)),
    (2, 5, '다음 주 팬미팅 일정 공지 드립니다. 꼭 확인해주세요!',       7,  2, DATE_ADD(NOW(), INTERVAL -6 HOUR));

-- ECHO 피드 3개 (artist_member_id=9 ECHO)
INSERT INTO artist_feed (artist_id, artist_member_id, content, like_count, comment_count, created_at)
VALUES
    (3, 9, 'ECHO 솔로 데뷔 앨범 발매 D-7. Limited Vinyl 예약 오픈!', 51, 20, DATE_ADD(NOW(), INTERVAL -7 DAY)),
    (3, 9, '레코딩 스튜디오에서 열심히 작업 중. 곧 만나요.',           14,  4, DATE_ADD(NOW(), INTERVAL -2 DAY)),
    (3, 9, '오늘 인스타 라이브 예정! 솔로 앨범 수록곡 일부 공개됩니다.', 8,  1, DATE_ADD(NOW(), INTERVAL -3 HOUR));
...
VALUES
    (1, null, 'NOVA 새 앨범 발매',             'DROP',   DATE_ADD(NOW(), INTERVAL 7 DAY), null),
    (1, null, 'NOVA 컴백 라이브 방송',          'LIVE',   DATE_ADD(NOW(), INTERVAL 10 DAY), 'https://live.example.com/nova'),
    (1, null, 'NOVA 팬미팅 — 서울 월드컵경기장', 'EVENT',  DATE_ADD(NOW(), INTERVAL 21 DAY), null),
    (1, null, '공식 팬클럽 멤버십 갱신 안내',    'NOTICE', DATE_ADD(NOW(), INTERVAL -2 DAY), null);
...
VALUES (1, DATE_ADD(CURRENT_DATE, INTERVAL -14 DAY), DATE_ADD(CURRENT_DATE, INTERVAL 16 DAY),
        '매일 출석 체크하고 포인트를 모아보세요!', true, NOW());

-- 8. 알림 (fan_id=1)
INSERT INTO notification (fan_id, notification_type, target_id, message, is_read, sent_at)
VALUES
    (1, 'NEW_FEED',        1, 'NOVA가 새 피드를 올렸어요.',    false, DATE_ADD(NOW(), INTERVAL -2 HOUR)),
    (1, 'ARTIST_SCHEDULE', 1, 'NOVA 컴백 일정이 등록됐어요.', false, DATE_ADD(NOW(), INTERVAL -1 HOUR));
...
VALUES
    (1, 'ord00000-0000-0000-0000-000000000001', 'opk_seed_0000000000000000000000000001', 'COMPLETED',    30000.00, DATE_ADD(NOW(), INTERVAL -7 DAY), DATE_ADD(NOW(), INTERVAL -7 DAY)),
    (1, 'ord00000-0000-0000-0000-000000000002', 'opk_seed_0000000000000000000000000002', 'COMPLETED',    29000.00, DATE_ADD(NOW(), INTERVAL -3 DAY), DATE_ADD(NOW(), INTERVAL -3 DAY)),
    (1, 'ord00000-0000-0000-0000-000000000003', 'opk_seed_0000000000000000000000000003', 'RESERVED', 45000.00, DATE_ADD(NOW(), INTERVAL -2 HOUR), DATE_ADD(NOW(), INTERVAL -2 HOUR));
...
INSERT INTO notification (fan_id, notification_type, target_id, message, is_read, sent_at)
VALUES
    (1, 'RESTOCK',         1, 'NOVA 한정판 포스터가 재입고됐어요!',      true,  DATE_ADD(NOW(), INTERVAL -2 DAY)),
    (1, 'PAYMENT_SUCCESS', 1, 'ECHO Limited Vinyl 결제가 완료됐어요.',    false, DATE_ADD(NOW(), INTERVAL -3 DAY)),
    (1, 'NEW_COMMENT',     1, 'NOVA 피드에 내 댓글에 답글이 달렸어요.',   true,  DATE_ADD(NOW(), INTERVAL -5 HOUR));
...
VALUES (2, 'LUNA 드롭스 미니앨범 패키지', 49000, 'ON_SALE',
        DATE_ADD(NOW(), INTERVAL 2 DAY), DATE_ADD(NOW(), INTERVAL 5 DAY));
...
VALUES (1, 'NOVA 스프링 드롭스 (만료)', 25000, 'SOLD_OUT',
        DATE_ADD(NOW(), INTERVAL -14 DAY), DATE_ADD(NOW(), INTERVAL -1 DAY));
...
VALUES
    (1, 'ord00000-0000-0000-0000-000000000004', 'opk_seed_0000000000000000000000000004',
     'COMPLETED', 59000.00, DATE_ADD(NOW(), INTERVAL -14 DAY), DATE_ADD(NOW(), INTERVAL -14 DAY)),
    (1, 'ord00000-0000-0000-0000-000000000005', 'opk_seed_0000000000000000000000000005',
     'COMPLETED', 29000.00, DATE_ADD(NOW(), INTERVAL -5 DAY), DATE_ADD(NOW(), INTERVAL -5 DAY)),
    (1, 'ord00000-0000-0000-0000-000000000006', 'opk_seed_0000000000000000000000000006',
     'CANCELLED', 35000.00, DATE_ADD(NOW(), INTERVAL -4 DAY), DATE_ADD(NOW(), INTERVAL -4 DAY));
...
SELECT o.id, 'toss_SEED_ORD4_CONFIRM_PAY_KEY_01', 59000, '카드', 'SUCCESS',
       DATE_ADD(NOW(), INTERVAL -14 DAY), null, DATE_ADD(NOW(), INTERVAL -14 DAY), 0
...
SELECT o.id, 'toss_SEED_ORD5_CONFIRM_PAY_KEY_01', 29000, '카드', 'SUCCESS',
       DATE_ADD(NOW(), INTERVAL -5 DAY), null, DATE_ADD(NOW(), INTERVAL -5 DAY), 0
...
SELECT o.id, 'toss_SEED_ORD6_FAILED_PAY_KEY_001', 35000, '카드', 'FAILED',
       null, DATE_ADD(NOW(), INTERVAL -4 DAY), DATE_ADD(NOW(), INTERVAL -4 DAY), 0
...
SELECT 'STORE', 'NOVA 드롭스 지금 구매하기',
       ...
       DATE_ADD(NOW(), INTERVAL -1 DAY), DATE_ADD(NOW(), INTERVAL 3 DAY), id
...
VALUES
    ('STORE', '[비활성] 구 스토어 기획전',
     ...
     DATE_ADD(NOW(), INTERVAL -30 DAY), DATE_ADD(NOW(), INTERVAL 20 DAY));
...
SELECT 'STORE', 'ECHO Limited Vinyl 마지막 특가',
       ...
       DATE_ADD(NOW(), INTERVAL -3 DAY), DATE_ADD(NOW(), INTERVAL 14 DAY), id
...
INSERT INTO feed_like (feed_id, fan_id, artist_member_id, artist_id, created_at)
SELECT id, 1, null, null, DATE_ADD(NOW(), INTERVAL 10 MINUTE)
FROM artist_feed
...
INSERT INTO comment (feed_id, artist_id, fan_id, artist_member_id, parent_id, content, created_at)
SELECT id, 1, 1, null, null,
       '오늘 방송 꼭 볼게요! 응원해요', DATE_ADD(NOW(), INTERVAL 2 MINUTE)
FROM artist_feed
WHERE content LIKE '오늘 라이브 방송%' AND artist_id = 1;
...
SELECT c.feed_id, c.artist_id, null, 1, c.id,
       '고마워요! 꼭 같이 즐겨요', DATE_ADD(NOW(), INTERVAL 15 MINUTE)
FROM comment c
...
SELECT id, 1, DATE_ADD(NOW(), INTERVAL 20 MINUTE)
FROM comment
...
VALUES (2, 'LUNA 미니앨범 굿즈 디자인 투표', DATE_ADD(NOW(), INTERVAL 5 DAY), true, NOW());
...
INSERT INTO goods_vote (artist_id, title, ends_at, is_active, created_at)
VALUES (1, 'NOVA 데뷔 기념 굿즈 투표 (종료)', DATE_ADD(NOW(), INTERVAL -3 DAY), false,
...
VALUES
    (2, null, 'LUNA 팬미팅 온라인 라이브',          'LIVE',
     DATE_ADD(NOW(), INTERVAL 5 DAY),  'https://live.example.com/luna', null, null),
    (2, null, 'LUNA 드롭스 미니앨범 오픈',           'DROP',
     DATE_ADD(NOW(), INTERVAL 2 DAY),  null, null, null),
    (3, null, 'ECHO 단독 콘서트 — 서울',            'EVENT',
     DATE_ADD(NOW(), INTERVAL 14 DAY), null, null, 'https://ticket.example.com/echo-concert'),
...
VALUES
    ...
    (5, 3, 'SOLAR', 1250, DATE_ADD(NOW(), INTERVAL -10 DAY), 'https://placehold.co/150x150/FFE066/333333?text=SOLAR', '스타뮤직의 차세대 5인조 하이틴 아이돌 SOLAR ☀️'),
    (6, 3, 'STELLA', 830, DATE_ADD(NOW(), INTERVAL -12 DAY), 'https://placehold.co/150x150/FFE066/333333?text=STELLA', '밤하늘의 가장 밝은 별빛처럼 노래하는 보컬 그룹 STELLA ✨'),
    (7, 3, 'VORTEX', 620, DATE_ADD(NOW(), INTERVAL -8 DAY), 'https://placehold.co/150x150/333333/FFFFFF?text=VORTEX', '강렬한 일렉트로닉 비트와 힙합 사운드로 무대를 압도하는 실력파 댄스 크루 VORTEX 🌪️'),

    -- 샌드박스 크리에이티브 (agency_id=4): 크리에이터 4명 (얼굴 공개)
    (8, 4, '민우 Minwoo', 3400, DATE_ADD(NOW(), INTERVAL -15 DAY), 'https://placehold.co/150x150/A0D2EB/333333?text=Minwoo', '최신 IT 기기 리뷰와 일상 브이로그를 전해드리는 민우입니다! 💻'),
    (9, 4, '소영 Soyoung', 4120, DATE_ADD(NOW(), INTERVAL -14 DAY), 'https://placehold.co/150x150/FFAAA6/333333?text=Soyoung', '데일리 메이크업 꿀팁과 유니크한 데일리룩 스타일링 소영 💄'),
    (10, 4, '동현 Donghyun', 2850, DATE_ADD(NOW(), INTERVAL -20 DAY), 'https://placehold.co/150x150/333333/FFFFFF?text=Donghyun', '종합 게임 방송과 유쾌한 입담으로 소통하는 동현의 게이밍 채널 🎮'),
    (11, 4, '혜린 Hyerin', 3080, DATE_ADD(NOW(), INTERVAL -9 DAY), 'https://placehold.co/150x150/D4A5A5/FFFFFF?text=Hyerin', '맛있는 쿡방과 신선한 레시피로 힐링을 주는 혜린의 달콤한 키친 🍳'),

    -- 브이유니버스 (agency_id=5): 버튜버 4명
    (12, 5, '리아 Lia', 4780, DATE_ADD(NOW(), INTERVAL -11 DAY), 'https://placehold.co/150x150/957DAD/FFFFFF?text=Lia', '노래하는 파란 머리 요정 🧚‍♀️ 버튜버 싱어송라이터 리아입니다 🎵'),
    (13, 5, '하루 Haru', 3590, DATE_ADD(NOW(), INTERVAL -13 DAY), 'https://placehold.co/150x150/E8D7FF/333333?text=Haru', '고양이 귀를 가진 츤데레 겜돌이 하루의 쉼터 🐱🎮'),
    (14, 5, '셀레네 Selene', 5120, DATE_ADD(NOW(), INTERVAL -16 DAY), 'https://placehold.co/150x150/B2F7EF/333333?text=Selene', '마법 도서관의 200세 대마법사! 인간 세상 게임 탐방기 셀레네 🔮'),
    (15, 5, '네오 Neo', 4050, DATE_ADD(NOW(), INTERVAL -7 DAY), 'https://placehold.co/150x150/333333/FF007F?text=Neo', '네온사인 번쩍이는 가상 도시에서 온 Cyberpunk DJ 네오 🎧⚡');
...
INSERT INTO artist_feed (artist_id, artist_member_id, content, like_count, comment_count, created_at)
VALUES
    (5, 10, 'SOLAR의 데뷔 앨범 [SUN] 예약 판매가 드디어 시작되었습니다! ☀️ 정말 열심히 준비했으니 많이 사랑해주세요!', 154, 42, DATE_ADD(NOW(), INTERVAL -2 DAY)),
    (6, 12, '오늘 밤 9시, 유튜브 라이브에서 만나요! 별빛들과 소통할 생각에 두근두근거리네요 ✨', 210, 56, DATE_ADD(NOW(), INTERVAL -5 HOUR)),
    (7, 14, 'VORTEX의 새로운 스트릿 퍼포먼스 영상이 곧 업로드됩니다. 다들 기대하셔도 좋습니다! 🌪️🔥', 89, 21, DATE_ADD(NOW(), INTERVAL -1 DAY)),
    (8, 15, '맥북 프로 M4 16인치 개봉기 & 한달 사용기! 프로필 링크에서 만나보세요. 💻', 320, 88, DATE_ADD(NOW(), INTERVAL -3 DAY)),
    (9, 16, '올리브영 세일 추천템! 건성 피부를 위한 보습 정착템들만 모아봤어요 💄✨', 412, 115, DATE_ADD(NOW(), INTERVAL -4 DAY)),
    (10, 17, '스팀 신작 생존 공포 게임 켠왕 갑니다!! 다들 야식 준비하시고 이따 8시에 만나요 🎮🍿', 289, 64, DATE_ADD(NOW(), INTERVAL -2 HOUR)),
    (11, 18, '여름철 입맛을 돋우는 수제 레몬 딜 버터 만들기 🍋 이번 주 레시피 노트 공개합니다!', 189, 32, DATE_ADD(NOW(), INTERVAL -5 DAY)),
    (12, 19, '리아의 첫 EP 타이틀곡 [Virtual Voice] 티저 영상이 공개되었습니다 🧚‍♀️ 많이 들어주실 거죠?', 540, 122, DATE_ADD(NOW(), INTERVAL -1 DAY)),
    (13, 20, '오늘 롤 랭크 게임 방송 도중 펜타킬 달성!! 하이라이트 클립 올려봅니다 후후 🐱🎮', 290, 78, DATE_ADD(NOW(), INTERVAL -4 HOUR)),
    (14, 21, '마법 다이어리 속 특별 스펠 🔮 오늘 하루도 평화롭길 바라요. 마법 가루 뾰로롱~', 380, 94, DATE_ADD(NOW(), INTERVAL -6 DAY)),
    (15, 22, '사이버 펑크 테크노 리믹스 셋업 완료! DJ 네오의 가상 클럽 파티 시작합니다 🎧⚡', 241, 51, DATE_ADD(NOW(), INTERVAL -3 DAY));
...
INSERT INTO comment (feed_id, artist_id, fan_id, artist_member_id, parent_id, content, created_at)
SELECT c.feed_id, c.artist_id, null, 10, c.id, '감사합니다! 정말 열심히 할게요 ☀️', DATE_ADD(NOW(), INTERVAL 10 MINUTE)
...
INSERT INTO comment (feed_id, artist_id, fan_id, artist_member_id, parent_id, content, created_at)
SELECT c.feed_id, c.artist_id, null, 19, c.id, '신비한 파란 목소리 마음에 드셨다니 다행이에요 💙', DATE_ADD(NOW(), INTERVAL 15 MINUTE)
FROM comment c WHERE c.content = '목소리 너무 신비롭고 예뻐요 ㅠㅠ 무한반복 중!' AND c.fan_id = 1;

-- 9. 굿즈 투표 (goods_vote & goods_vote_option)
-- SOLAR 투표
INSERT INTO goods_vote (artist_id, title, ends_at, is_active, created_at)
VALUES (5, 'SOLAR 첫 공식 응원봉 컬러 투표 ☀️', DATE_ADD(NOW(), INTERVAL 10 DAY), true, NOW());
...
-- 셀레네 투표
INSERT INTO goods_vote (artist_id, title, ends_at, is_active, created_at)
VALUES (14, '셀레네 가죽 다이어리 커버 각인 문양 선택 🔮', DATE_ADD(NOW(), INTERVAL 5 DAY), true, NOW());
...
VALUES
    (5, null, 'SOLAR 데뷔 앨범 발매 쇼케이스', 'LIVE', DATE_ADD(NOW(), INTERVAL 3 DAY), 'https://live.starmusic.example.com/solar', 'SOLAR의 역사적인 데뷔 무대! 실시간 쇼케이스 방송에 많은 참여 부탁드립니다.'),
    (8, null, '민우 IT 데스크 셋업 실시간 Q&A', 'LIVE', DATE_ADD(NOW(), INTERVAL 1 DAY), 'https://youtube.com/minwoo/live', '여러분들이 많이 질문해주셨던 컴퓨터 및 조명 세팅에 대해 실시간 소통으로 답변 드립니다.'),
    (12, null, '[공지] 리아 첫 단독 가상 콘서트 [Virtual Sky]', 'NOTICE', DATE_ADD(NOW(), INTERVAL -2 DAY), null, '리아의 첫 버추얼 콘서트가 메타버스 플랫폼에서 열립니다. 자세한 예매 사이트 및 접속 방법은 추후 공지될 예정입니다.');
