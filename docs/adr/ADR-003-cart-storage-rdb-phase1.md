...
| 관점 | Redis | RDB |
| --- | --- | --- |
| 성능 | 메모리 기반으로 읽기/쓰기 빠름, 핫딜 담기 트래픽을 DB 앞단에서 흡수 가능 | `cart_item` INSERT/UPDATE는 상대적으로 가벼우나, 주문·재고와 **동일 커넥션 풀** 경합 가능 |
...
| UX | 재시작·장애 시 담기 목록 소실 → 핫딜에서 **재담기 부담** 큼 | 로그인 사용자 **PC/모바일 간 목록 유지** 기대에 부합 |

프로젝트 원칙은 **측정 후 튜닝**이다. 핫딜 순간 실제 병목 후보는 대체로 다음 순이다.
...
k6 핫딜 부하 테스트(Phase 4)에서 아래 **하나 이상**이 확인되면 Hybrid(예: Redis = 실시간 조작, RDB = 영속 스냅샷) 전환을 **ADR 개정**으로 검토한다.

| # | 트리거 | 임계치 (합의) |
| --- | --- | --- |
| T1 | `cart_item` write P95 | **주문 생성 트랜잭션 P95의 20% 이상**을 차지 |
| T2 | DB 커넥션 풀 점유율 | 핫딜 구간 **80% 초과**가 **5분 이상** 지속 |
| T3 | HikariCP active connections | `maximum-pool-size`에 **근접**(운영 설정값 기준, 예: ≥90%) |

트리거 미충족 시 **RDB 유지**. Redis는 [ADR-001](./ADR-001-multi-module-monolith.md)대로 **대기열·캐시·랭킹** 등 기존 용도만 사용한다.
...
| Redis Phase 1 | **핫딜 대기열**(F08-01), Read 캐시, 랭킹 등 — **장바구니 아님** |
