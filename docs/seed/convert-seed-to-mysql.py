#!/usr/bin/env python3
"""Convert local H2 test-data.sql to MySQL RDS demo seed."""
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
SRC = ROOT / "apps/api-server/src/main/resources/seed/test-data.sql"
DEST = Path(__file__).resolve().parent / "demo-seed-prod-mysql.sql"

text = SRC.read_text(encoding="utf-8")

# Remove shedlock CREATE (schema from Flyway on RDS)
text = re.sub(r"-- 0\. ShedLock.*?\);\n\n", "", text, flags=re.DOTALL)

def repl_dateadd(m: re.Match) -> str:
    unit = m.group(1)
    n = int(m.group(2))
    base = "NOW()" if m.group(3).strip() == "NOW()" else "CURDATE()"
    if n < 0:
        return f"DATE_SUB({base}, INTERVAL {abs(n)} {unit})"
    return f"DATE_ADD({base}, INTERVAL {n} {unit})"

text = re.sub(
    r"DATEADD\('(\w+)',\s*(-?\d+),\s*(NOW\(\)|CURRENT_DATE)\)",
    repl_dateadd,
    text,
)
text = text.replace("CURRENT_DATE", "CURDATE()")

text = re.sub(r"^-- Local dev seed.*?\n-- =+\n\n", "", text, count=1)
text = re.sub(
    r"^-- =+\n-- 테스트 계정.*?\n-- =+\n\n",
    "",
    text,
...
BC = "$2a$10$IXraSx3hpYkrj8jRqqIsxOkdIfRCZKYxPafAJT7v3ZrlvB43gl7Re"
replacements = [
    (
        f"VALUES (1, 'agency@fandrops.test',\n        '{BC}',\n        '테스트 기획사', 'agency@fandrops.test', 'ACTIVE', 'AGENCY', NOW());",
    ...
    (
        f"VALUES (2, 'fan2@fandrops.test', '테스트팬2', 'LOCAL', 'local-fe-2',\n        '{BC}',\n        true, NOW());",
        f"VALUES (2, 'fan2@fandrops.test', '테스트팬2', 'LOCAL', 'local-fe-2',\n        '{BC}',\n        true, NOW())\nON DUPLICATE KEY UPDATE email=VALUES(email), nickname=VALUES(nickname), password_hash=VALUES(password_hash);",
    ),
]
...
remaining = len(re.findall(r"DATEADD\(", out))
print(f"Written {DEST} ({len(out.splitlines())} lines), remaining DATEADD: {remaining}")
