...
import com.fandrops.community.application.schedule.NoticeCreateCommand;
...
public class ScheduleController extends CommunityControllerSupport {
    ...
    // POST /api/v1/artists/{artistId}/notices — 공지 등록 (AGENCY 권한)
    @PostMapping("/api/v1/artists/{artistId}/notices")
    ...
    // GET /api/v1/artists/{artistId}/notices — 공지 목록 커서 페이지네이션 (공개)
    @GetMapping("/api/v1/artists/{artistId}/notices")
    public ResponseEntity<ApiResponse<NoticeListResult>> getNotices(
            @PathVariable Long artistId,
            @RequestParam(required = false) String cursor,
            @RequestParam(defaultValue = "20") @Min(1) @Max(50) int size) {

        NoticeListResult result = scheduleService.getNotices(artistId, cursor, size);
        return ResponseEntity.ok(ApiResponse.ok(result, traceId()));
    }

    // GET /api/v1/artists/{artistId}/notices/{noticeId} — 공지 단건 조회 (공개)
    @GetMapping("/api/v1/artists/{artistId}/notices/{noticeId}")
    public ResponseEntity<ApiResponse<NoticeResult>> getNotice(
            @PathVariable Long artistId,
